    ...
    popup.confirm('what format?', 'the .pxlo format can only be opened by this editor, use it if you are not done yet.<br>The .png format can be opened by image veiwers and shared with your freinds but CANNOT be opened by this program.', '.pxlo', '.png',
      ...
      function () {
        ...
        button.addEventListener('click', function (e) {
          const dataURL = saveCanvas.toDataURL('image/png').replace('image/png', 'image/octet-stream')
          button.href = dataURL
...
function paintBucket (event) {
  let x = event.pageX - $('#drawingCanvas').offset().left // get the x pos relitive to the canvas
  let y = event.pageY - $('#drawingCanvas').offset().top
  x /= pixelSize
  x = Math.floor(x)
  y /= pixelSize
  y = Math.floor(y)
